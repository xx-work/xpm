...
from manager.api.center.models import SystemPolicyCentralizedManagement, AuditPolicyCentralizedManagement, SecurityPolicyCentralizedManagement
from manager.api.xobj.models import SysManagerCopInfo, ConnectManagerUserInfo, AuditLogObject
from manager.api.policy.models import PolicyActionHistory, PolicyRule, PolicyAction, PolicyBench
from manager.api.monitor.models import ObjProcess, ProcessAuditLog
from manager.api.solution.models import InfoSecEvent, InfoGoin, AttackerActionDesc, EffectInfo
from manager.api.chk.models import ChangeAudit
from manager.api.backer.models import BackUpHistory

from agent.snmpd.models import SnmpAgentCfgInfo, SnmpHostData
from secs.models import Community
    ...
    def get_site_menu(self):
        ...
        if self.request.user.is_superuser:
            return (
                ...
                # {'title': '定时任务', 'menus': (
                #     {'title': '周期任务', 'url': self.get_model_url(PeriodicTask, 'changelist')},
                #     {'title': 'Crontab任务', 'url': self.get_model_url(CrontabSchedule, 'changelist')},
                #     {'title': '间隔周期任务', 'url': self.get_model_url(IntervalSchedule, 'changelist')},
                # ), "icon": "fa fa-task"},
                # {'title': '', 'menus': (), "icon": "fa fa-ignore"},
...
from manager.xadmin.plugins.tplugin import ListReturndPlugin
